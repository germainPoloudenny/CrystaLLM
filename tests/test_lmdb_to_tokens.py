...
def main(args):
    ...
    # Load optional metadata from the root database
    num_embeddings = None
    env = lmdb.open(args.lmdb_path, readonly=True, lock=False, max_dbs=128)
    with env.begin() as txn:
        raw = txn.get(b"num_embeddings")
        if raw is not None:
            try:
                num_embeddings = pickle.loads(raw)
                if hasattr(num_embeddings, "__len__"):
                    num_embeddings = int(np.max(num_embeddings))
                else:
    ...
    max_token = max(int(np.max(s)) for s in sequences)
    if num_embeddings is not None and max_token >= num_embeddings:
        raise ValueError(
            f"Token id {max_token} exceeds num_embeddings ({num_embeddings})"
        )

    print(f"Detected sequence length = {seq_len}")
    if num_embeddings is not None:
        print(f"LMDB reports num_embeddings = {num_embeddings}")
